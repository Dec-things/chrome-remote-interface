...
class Chrome extends EventEmitter {
    ...
    async _start() {
        ...
        try {
            if (this.transport) {
                const protocol = await this.transport.getProtocol();
                api.prepare(this, JSON.parse(protocol));
    ...
    _enqueueCommand(method, params, cb) {
        ...
        if (this.transport) {
            ...
            this._ws.send(JSON.stringify(message), callback);
        }
    }
}
